    ...
    <table class="table service">
      ...
      <tr v-for="item in items" :key="item.id" class="h-20">
        <th class="content-center">
          <check-circle-icon class="h-8 w-8 text-green-500"/>
        </th>
        <td>
        ...
        <td>
          <div class="grid grid-rows-1 grid-flow-col items-end">
            <div v-for="data in item.chartData" :key="data.id" class="w-1 bg-green-500 mr-0.5 tooltip" v-bind:class="{'bg-yellow-500': data.isDegraded, 'bg-red-500': data.hasErrors || data.hasFailures}" v-bind:style="{height: (0.5 + 1.5 * data.v) + 'rem'}" v-bind:data-tip="data.responseTime + 'ms'"></div>
          </div>
        </td>
